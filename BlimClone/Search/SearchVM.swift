...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let defEpiInfo = CurrentEpisodeInfo(episodeName: epiname, desc: epidesc, season: 1, episode: 1)
        
        let allTrailers = generateTrailers(3)
        
        let moreLikeThis = exampleMovies
        
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, cagetories: categories, year: year, rating: "PG-13", numberOfSeasons: numSeasons, episodes: allEpisodes, promotionHeadLine: headline, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creators: creators, cast: cast, moreLikeThisMovies: moreLikeThis, trailers: allTrailers)
...
func generateTrailers(_ count: Int) -> [Trailer] {
    ...
    for _ in 0..<count {
        ...
        let trailer = Trailer(id: id, name: name, videoURL: randomExampleVideoURL, thumbnailImageURL: thumbnail)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImageURLString: thumbnail, desc: desc, length: randLength, videoURL: randomExampleVideoURL)
